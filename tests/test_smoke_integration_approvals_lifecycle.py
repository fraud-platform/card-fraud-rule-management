...
async def test_submit_creates_pending_approval_and_audit(maker_client, admin_client):
    ...
    create_resp = maker_client.post("/api/v1/rules", json=payload)
    assert create_resp.status_code == 201
    rule_id = create_resp.json()["rule_id"]

    # Create new version
    version_payload = {"condition_tree": payload["condition_tree"], "priority": 100}
    version_resp = maker_client.post(f"/api/v1/rules/{rule_id}/versions", json=version_payload)
    assert version_resp.status_code == 201
    version_id = version_resp.json()["rule_version_id"]

    # Submit for approval
    submit_resp = maker_client.post(f"/api/v1/rule-versions/{version_id}/submit", json={})
    assert submit_resp.status_code == 200

    # Check approvals list contains pending approval
    ap_resp = maker_client.get("/api/v1/approvals")
...
async def test_approve_and_reject_flow(maker_client, checker_client, admin_client):
    ...
    maker_client.post(f"/api/v1/rule-versions/{version_id}/submit", json={})

    # Approve as checker
    approve_resp = checker_client.post(f"/api/v1/rule-versions/{version_id}/approve", json={})
    assert approve_resp.status_code == 200
    approved = approve_resp.json()
    assert approved["status"] == "APPROVED"

    # Check approval row updated
    ap_resp = checker_client.get("/api/v1/approvals")
    approvals_data = ap_resp.json()
    approvals = approvals_data["items"]
    assert any(
        a
    ...
    audit_resp = admin_client.get("/api/v1/audit-log?action=APPROVE&entity_type=RULE_VERSION")
    ...
    version_resp2 = maker_client.post(f"/api/v1/rules/{rule_id}/versions", json=version_payload)
    version2 = version_resp2.json()["rule_version_id"]
    maker_client.post(f"/api/v1/rule-versions/{version2}/submit", json={})

    reject_resp = checker_client.post(f"/api/v1/rule-versions/{version2}/reject", json={})
    assert reject_resp.status_code == 200
    rejected = reject_resp.json()
    assert rejected["status"] == "REJECTED"

    # Check audit log for REJECT
    audit_reject_data = checker_client.get(
        "/api/v1/audit-log?action=REJECT&entity_type=RULE_VERSION"
    ).json()
